...
%
%% Optional input data:
%      fun     function f(u) to evaluate ( 'exp', 'expdamp' (default) , 'gauss', 'gaussdamp' )
...
% Reference: von Papen, M. and J. Saur, Forward Modeling of Power Spectra
...
function P = cb_psd_parf (f, theta, varargin)
...
%% Set output variable
if ratioTP ~=0
    Ptmp = zeros(4,length(kix));
    P    = zeros(length(kix),ntheta,4);
else
    Ptmp = zeros(length(kix));
...
for k=1:ntheta
  ...
  parfor i=1:nf
    ...
    if ratioTP ~= 0
      ...
      Ptmp(:,i) = [integral_dkxy( ky.^2.*Tor + (kx.*kz).^2 ./ kp2 .* Pol );
                   integral_dkxy( kx.^2.*Tor + (ky.*kz).^2 ./ kp2 .* Pol );
                   integral_dkxy( kp2 .* Pol )];
%       Ptmp(4,i) = sum(Ptmp(1:3,i));
