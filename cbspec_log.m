...
function [P,kern] = cbspec_log (f,theta,n,fun,L0,rho,v,va,B,bounds,ratioTP,rhoe,si_in,cb_in,mirror)
...
%         cbetaT=cbetaT
        ...
        for L=L0
            ...
            switch fun
                case 1 %'exp'
                    kern( i1 ) = log(kp2(i1))*(si/2)...
                        -L^(1-cb).*abs(kz(i1))./kp2(i1).^(cb/2)...
                        +log(dky(i1))+log(dkiz(i1));
            ...
            switch fun
                case 1 %'exp'
                    kern( i1 ) = log(rho)*(si2-si)+log(kp2(i1))*(si2/2)...
                        -L^(1-cb)*rho^(cb-cb2)*abs(kz(i1))./kp2(i1).^(cb2/2)...
                        +log(dky(i1))+log(dkiz(i1));
                case 2 %'expdamp' for Ti=Te => rho_e=sqrt(me/mi)*rho_i
        ...
        if ratioTP ~= 0
            ...
            if mirror~=0
                sigma=0.2; sigmaz=1e-7/rho;
                i1=find(kz.^2./kp2<1e-2);
                Pkern(i1)=Pkern(i1)+mirror/(2*pi*sqrt(sigma*sigmaz))...
                    *exp(-log10(10*sqrt(kp2(i1))*rho).^2/2/sigma^2 ...
                         -kz(i1).^2/2/sigmaz).*dky(i1).*dkiz(i1);
            end
            
            Pol=(1-ratioTP/(1+ratioTP))*Pkern./kabs2;
